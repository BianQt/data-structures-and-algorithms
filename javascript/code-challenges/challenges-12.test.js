...
const replaceZeros = (string) => {
  // Solution code here...
  const numToReplace = /[0]/g;
  const wordToPut = "zero";

  return string.replace(numToReplace, wordToPut);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let val = /^((\(\d{3}\))|(\d{3}))[ -]?\d{3}[ -]?\d{4}$/;

  if (val.test(phoneNumber)) {
    return true;
  } else {
    return false;
  }
